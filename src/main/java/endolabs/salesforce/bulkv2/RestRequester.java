    ...
    private <T> T request(String url, String httpMethod, Map<String, String> queryParams, RequestBody requestBody, Class<T> responseClass) {
        ...
        try (Response response = doRequest(request)) {
            ...
            if (response.isSuccessful()) {
                return (body1 == null || body1.isEmpty()) ? null : Json.decode(body1, responseClass);
            } else {
                try {
                    List<ErrorResponse> errors = Json.decode(body1, ERRORS_TYPE_REFERENCE);
                    throw new BulkRequestException(errors);
                } catch (UncheckedIOException e) {
                    throw new BulkRequestException("Unknown error: " + body1);
                }
            }
        }catch(Exception e){
